...
description: "Flutter 저장소에서 세션 인계 작업을 수행한다. 사용자가 세션 인계을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/52-session-handoff.md)
...
2. 현재 브랜치·커밋·미커밋 변경·계획을 대조한다
3. 완료·미완료·실패·검증·위험을 증거와 함께 기록한다
4. 다음 세션의 첫 명령과 필요한 환경을 구체적으로 적는다
...
**갱신된 계획·작업 상태·다음 행동**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
