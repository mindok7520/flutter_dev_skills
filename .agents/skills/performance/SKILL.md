---
name: performance
description: "Flutter 저장소에서 성능 점검 작업을 수행한다. 사용자가 성능 점검을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/13-performance-audit.md)
...
2. 실행 모드·기기·시나리오·기준선을 확인한다
3. CPU·GPU·네트워크·메모리 병목을 분리한다
4. 한 가설씩 측정 가능한 개선을 제안·검증한다
...
**환경·원본 추적·전후 분포·회귀 위험**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
