...
description: "Flutter 저장소에서 보안 검토 작업을 수행한다. 사용자가 보안 검토을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/11-security-review.md)
...
2. 자산·입력·신뢰 경계를 확인한다
3. 인증·권한·비밀·로그·공급망·저장소를 조사한다
4. 실제 공격 경로와 방어의 빈틈을 증거로 제시한다
...
**위험·공격 조건·영향·완화·검증**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
