---
name: web-wasm
description: "Flutter 저장소에서 Wasm 검토 작업을 수행한다. 사용자가 Wasm 검토을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/22-wasm-review.md)
...
2. 사용 SDK와 플러그인의 Wasm 지원을 공식 자료로 확인한다
3. 기본·Wasm 빌드의 실행·fallback·interop를 검증한다
4. 속도·시작·크기·메모리 이득과 호환 비용을 비교한다
...
**버전·지원 표·측정·채택 또는 보류 근거**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
