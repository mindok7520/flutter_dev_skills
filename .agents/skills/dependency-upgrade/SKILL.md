...
description: "Flutter 저장소에서 의존성 갱신 작업을 수행한다. 사용자가 의존성 갱신을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/25-dependency-upgrade.md)
...
2. 현재 잠금 버전과 목표 버전의 변경 기록을 비교한다
3. 호환성·보안·네이티브 최소 버전·코드 생성을 검토한다
4. 좁은 범위로 갱신하고 잠금 파일·회귀·문서를 확인한다
...
**버전 차이·마이그레이션·검증·되돌림**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
