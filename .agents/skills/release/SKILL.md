---
name: release
description: "Flutter 저장소에서 출시 후보 생성 작업을 수행한다. 사용자가 출시 후보 생성을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [RELEASE_WORKFLOW](../../../docs/workflow/RELEASE_WORKFLOW.md)
- [RELEASE_CHECKLIST](../../../docs/release/RELEASE_CHECKLIST.md)
- [작업 프롬프트](../../../prompts/48-release.md)
...
2. 검증한 후보 SHA와 허용된 환경을 확인한다
3. 버전·품질·정책·서명·복구 증거를 점검한다
4. 허용된 범위의 후보 생성 또는 배포를 수행하고 실제 결과를 확인한다
...
**SHA·산출물·해시·실행 결과·후속 관측**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
