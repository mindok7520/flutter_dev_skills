---
name: advertising
description: "Flutter 저장소에서 광고 구현 작업을 수행한다. 사용자가 광고 구현을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/34-ad-integration.md)
...
2. 지원 플랫폼·광고 형식·동의·테스트 ID를 확인한다
3. SDK 초기화·광고 수명·빈도·실패 UI를 구현한다
4. 거절·철회·오프라인·재진입·메모리를 검증한다
...
**배치·동의 흐름·코드·수명 검증**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
