---
name: concurrency
description: "Flutter 저장소에서 동시성 검토 작업을 수행한다. 사용자가 동시성 검토을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/16-concurrency-review.md)
...
2. await 사이 상태 변화와 응답 역전을 추적한다
3. 중복·취소·타임아웃·큐 상한·dispose·종료를 검토한다
4. 최신 요청 우선 또는 직렬화 등 필요한 정책을 구현한다
...
**수명 지도·경합 재현·정책·테스트**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
