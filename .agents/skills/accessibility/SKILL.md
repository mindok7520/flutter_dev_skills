---
name: accessibility
description: "Flutter 저장소에서 접근성 검토 작업을 수행한다. 사용자가 접근성 검토을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/20-accessibility-review.md)
...
2. 의미 레이블·대비·터치 영역을 자동 검사한다
3. 키보드·TalkBack·VoiceOver·큰 글자로 핵심 흐름을 수행한다
4. 자동 검증과 수동 검증·미실행을 구분한다
...
**항목별 근거·실패·수정·수동 검증 범위**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
