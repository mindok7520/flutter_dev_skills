---
name: testing
description: "Flutter 저장소에서 테스트 계획 작업을 수행한다. 사용자가 테스트 계획을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/26-test-plan.md)
...
2. 수락 기준과 위협·실패 조건을 수집한다
3. 가장 낮은 유효 계층으로 시나리오를 배치한다
4. 환경·데이터·명령·기대 결과·완료 기준을 작성한다
...
**요구-테스트 연결·우선순위·환경·제외 근거**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
