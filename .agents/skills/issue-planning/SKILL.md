---
name: issue-planning
description: "Flutter 저장소에서 이슈 계획 작업을 수행한다. 사용자가 이슈 계획을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [TASK_EXECUTION_PROTOCOL](../../../docs/agent/TASK_EXECUTION_PROTOCOL.md)
- [작업 프롬프트](../../../prompts/04-plan-issue.md)
...
2. 이슈의 현재/기대 결과와 코드를 조사한다
3. 영향 파일·설계 결정·작은 구현 단계·실패 복구를 정한다
4. docs/exec-plans/active에 이슈 계획을 작성한다
...
**계획 파일·단계별 완료 조건·테스트·복구 경로**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
