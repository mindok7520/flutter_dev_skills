---
name: payments
description: "Flutter 저장소에서 결제 구현 작업을 수행한다. 사용자가 결제 구현을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/32-payment-integration.md)
...
2. 상품·국가·플랫폼·공급자를 공식 정책과 함께 확인한다
3. 서버 검증·멱등 원장·권리 반영·복원 계약을 구현한다
4. sandbox에서 취소·대기·중복·웹훅 역전·실패 복구를 검증한다
...
**설계·원장 상태 전이·코드·시험 증거**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
