---
name: code-review
description: "Flutter 저장소에서 코드 리뷰 작업을 수행한다. 사용자가 코드 리뷰을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/10-code-review.md)
...
2. 요구와 diff·호출 경로를 함께 확인한다
3. 정확성·권한·수명·데이터 손실을 우선 검토한다
4. 재현 조건·파일·행·영향·수정 제안으로 지적한다
...
**중요도별 발견·근거·검증 범위**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
