---
name: architecture
description: "Flutter 저장소에서 아키텍처 설계 작업을 수행한다. 사용자가 아키텍처 설계을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/05-architecture-design.md)
...
2. 요구·신뢰 경계·데이터 흐름·수명을 설계한다
3. 최소 두 대안과 현상 유지의 비용을 비교한다
4. 중요 결정은 ADR에 기록하고 최소 수직 구현을 정의한다
...
**구성도·계약·대안·ADR·검증 계획**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
