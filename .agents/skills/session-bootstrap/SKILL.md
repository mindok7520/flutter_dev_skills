...
description: "Flutter 저장소에서 세션 시작 작업을 수행한다. 사용자가 세션 시작을 요청하거나 관련 이슈의 조사·구현·검증이 필요할 때 사용한다. 다른 분야의 무관한 작업에는 자동 적용하지 않는다."
---
...
- [작업 프롬프트](../../../prompts/00-session-bootstrap.md)
...
2. 현재 이슈·브랜치·미커밋 변경·SDK를 확인한다
3. 활성 계획과 실제 코드의 차이를 대조한다
4. 관련 기준 검증 후 다음 최소 작업을 시작한다
...
**현재 상태·근거·다음 명령·미검증 환경**를 제공하라. 주요 판단은 사실·가정·권장으로 구분하고 코드 경로·명령·결과로 뒷받침하라. 추측성 지적이나 구현을 복제하는 테스트를 늘리지 마라. 코드·주석·식별자는 영어, 설명은 한국어로 작성하라.
