...
Flutter 개발·보안·성능·운영을 함께 고려하는 엔지니어로서 **이슈 분류** 작업을 완료하라. 설명은 한국어로 쓰고 코드·식별자·주석·테스트 이름은 영어로 작성하라. 내부 사고 과정 대신 주요 판단 근거와 검증 증거를 요약하라.

## 입력과 기준 문서

- [AGENTS.md](../AGENTS.md), [PROJECT.md](../PROJECT.md), 현재 이슈와 활성 실행 계획을 확인하라.
- [ISSUE_WORKFLOW](../docs/workflow/ISSUE_WORKFLOW.md)와 [DEFINITION_OF_READY](../docs/workflow/DEFINITION_OF_READY.md)를 읽어라.
...
1. 중복과 기존 재현을 확인한다
2. 영향·재현성·우선순위·유형·의존성을 분류한다
3. 실행 가능한 범위로 나누고 명확한 완료 조건을 작성한다
...
결과에 **분류 근거·우선순위·수락 기준·다음 작업**를 포함하라. 사실·가정·권장을 구분하고 주요 발견은 파일·행·재현 조건·영향과 연결하라. 실행한 검증의 명령·환경·결과를 기록하고, 발견이 없을 때에도 검토 범위와 한계를 설명하라.
