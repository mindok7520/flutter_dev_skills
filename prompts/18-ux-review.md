...
Flutter 개발·보안·성능·운영을 함께 고려하는 엔지니어로서 **사용 경험 검토** 작업을 완료하라. 설명은 한국어로 쓰고 코드·식별자·주석·테스트 이름은 영어로 작성하라. 내부 사고 과정 대신 주요 판단 근거와 검증 증거를 요약하라.

## 입력과 기준 문서

- [AGENTS.md](../AGENTS.md), [PROJECT.md](../PROJECT.md), 현재 이슈와 활성 실행 계획을 확인하라.
- [UX_GUIDELINES](../docs/design/UX_GUIDELINES.md)와 [USER_JOURNEYS](../docs/product/USER_JOURNEYS.md)를 읽어라.
- 현재 Git 상태·관련 코드·매니페스트·잠금 파일·테스트를 사실의 근거로 삼아라.
- 바뀔 수 있는 API·버전·보안·스토어 정책은 실제 사용 버전과 공식 출처·확인 날짜를 명시하라.

## 실행 절차

1. 핵심 작업을 처음부터 취소·재시도까지 수행한다
2. 로딩·오류·권한 거절·뒤로가기의 복구성을 검토한다
3. 사용자 비용과 완료율에 근거해 개선을 우선한다
...
결과에 **여정·막힘·영향·개선 순서**를 포함하라. 사실·가정·권장을 구분하고 주요 발견은 파일·행·재현 조건·영향과 연결하라. 실행한 검증의 명령·환경·결과를 기록하고, 발견이 없을 때에도 검토 범위와 한계를 설명하라.
