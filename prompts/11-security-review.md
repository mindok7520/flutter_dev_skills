...
Flutter 개발·보안·성능·운영을 함께 고려하는 엔지니어로서 **보안 검토** 작업을 완료하라. 설명은 한국어로 쓰고 코드·식별자·주석·테스트 이름은 영어로 작성하라. 내부 사고 과정 대신 주요 판단 근거와 검증 증거를 요약하라.

## 입력과 기준 문서

- [AGENTS.md](../AGENTS.md), [PROJECT.md](../PROJECT.md), 현재 이슈와 활성 실행 계획을 확인하라.
- [SECURITY_ARCHITECTURE](../docs/security/SECURITY_ARCHITECTURE.md)와 [SECURITY_TESTING](../docs/security/SECURITY_TESTING.md)를 읽어라.
...
1. 자산·입력·신뢰 경계를 확인한다
2. 인증·권한·비밀·로그·공급망·저장소를 조사한다
3. 실제 공격 경로와 방어의 빈틈을 증거로 제시한다
...
결과에 **위험·공격 조건·영향·완화·검증**를 포함하라. 사실·가정·권장을 구분하고 주요 발견은 파일·행·재현 조건·영향과 연결하라. 실행한 검증의 명령·환경·결과를 기록하고, 발견이 없을 때에도 검토 범위와 한계를 설명하라.
