...
Flutter 개발·보안·성능·운영을 함께 고려하는 엔지니어로서 **패키지 평가** 작업을 완료하라. 설명은 한국어로 쓰고 코드·식별자·주석·테스트 이름은 영어로 작성하라. 내부 사고 과정 대신 주요 판단 근거와 검증 증거를 요약하라.

## 입력과 기준 문서

- [AGENTS.md](../AGENTS.md), [PROJECT.md](../PROJECT.md), 현재 이슈와 활성 실행 계획을 확인하라.
- [PACKAGE_POLICY](../docs/engineering/PACKAGE_POLICY.md)와 [DEPENDENCY_SECURITY](../docs/security/DEPENDENCY_SECURITY.md)를 읽어라.
- 현재 Git 상태·관련 코드·매니페스트·잠금 파일·테스트를 사실의 근거로 삼아라.
- 바뀔 수 있는 API·버전·보안·스토어 정책은 실제 사용 버전과 공식 출처·확인 날짜를 명시하라.

## 실행 절차

1. SDK 내장 대안과 후보 패키지를 비교한다
2. 버전·라이선스·유지보수·취약점·플랫폼·Wasm을 확인한다
3. 작은 실험으로 크기·초기화·교체 비용을 측정한다
...
결과에 **후보 비교·공식 출처·측정·결정**를 포함하라. 사실·가정·권장을 구분하고 주요 발견은 파일·행·재현 조건·영향과 연결하라. 실행한 검증의 명령·환경·결과를 기록하고, 발견이 없을 때에도 검토 범위와 한계를 설명하라.
...
다운로드 수는 안전성 보증이 아니다. 패키지 하나가 가져오는 전이 의존성과 서버 비용까지 검토한다.
