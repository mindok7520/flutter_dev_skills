...
- APK·AAB·IPA·웹 원본과 압축 크기를 구분한다
- 같은 아키텍처·모드·분할 조건에서 비교한다
- 소스맵·심볼은 공개 번들 정책과 분리한다

## 실행 절차

1. flutter build의 크기 분석 기능과 산출물 바이트를 기록한다
2. 대형 패키지·폰트·중복 자산의 기여를 찾는다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 빌드 SHA와 도구 체인·압축 조건이 있다
- [ ] 크기 절감 후 런타임 로딩 오류가 없다
