...
- CDN 캐시와 브라우저 캐시를 구분한다
- 번들·폰트·이미지·Wasm을 별도 집계한다
- 처음 방문과 캐시된 재방문을 모두 본다

## 실행 절차

1. 배포 환경과 동일한 압축·헤더로 정적 빌드를 제공한다
2. 브라우저 네트워크·성능 도구로 타임라인을 수집한다
...
데스크톱 로컬 서버의 성능은 모바일 운영 성능을 대변하지 않는다. 용량 개선과 초기화 비용을 함께 비교한다.
