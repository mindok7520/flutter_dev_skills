...
- master에서 hotfix/번호-slug를 만든다
- 영향·재현·완화·롤백 가능성을 먼저 기록한다
- 긴급하더라도 결제·권한·데이터 검증을 생략하지 않는다

## 실행 절차

1. 최소 재현과 회귀 테스트를 작성한다
2. 시험 채널 검증 후 master에 반영하고 develop에도 동일 수정 또는 동등 수정 PR을 만든다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 실제 출시 버전에서 결함이 해결된다
- [ ] develop이 다음 출시 때 결함을 되살리지 않는다
