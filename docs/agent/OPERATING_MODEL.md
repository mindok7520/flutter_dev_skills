...
- AGENTS.md는 지도이고 분야 문서는 필요할 때 읽는다
- AI의 자신감 대신 코드·테스트·Git·공식 출처를 근거로 삼는다
- 작업 범위와 권한은 사용자 지시·도구 정책을 따른다

## 실행 절차

1. 이슈와 실행 계획을 복원한다
2. 작은 변경을 구현·검증·기록하고 다음 행동을 명시한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 설명에 사실·가정·미실행이 구분된다
- [ ] 변경 결과가 다른 세션에서도 재현된다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

긴 대화에만 상태를 두면 문맥 단절 때 손실된다. 저장소 기록은 짧게 유지하되 결정과 증거를 생략하지 않는다.
