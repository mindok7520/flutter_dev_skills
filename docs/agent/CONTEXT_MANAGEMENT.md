...
- 전체 파일 대신 관련 구간·호출 경로·요약을 읽는다
- 원본 증거는 파일·이슈·검증 로그 링크로 유지한다
- 비밀·개인정보를 문맥에 복사하지 않는다

## 실행 절차

1. 작업을 수락 기준별 작은 단위로 분리한다
2. 단위 종료마다 계획에 결정·검증·다음 행동을 갱신한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 요약만 읽어도 바로 다음 작업을 시작할 수 있다
- [ ] 핵심 제약과 가정이 누락되지 않는다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

과한 요약은 중요한 실패 조건을 잃는다. 숫자·버전·명령·경로·검증 결과는 정확히 보존한다.
