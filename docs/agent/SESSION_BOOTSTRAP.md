...
- AGENTS·PROJECT·활성 계획·Git 상태를 먼저 읽는다
- 이전 세션의 성공 결과를 현재 커밋에 자동 적용하지 않는다
- 모든 문서를 한 번에 로드하지 않는다

## 실행 절차

1. 브랜치·원격·SDK·잠금 파일·미커밋 변경을 확인한다
2. 관련 기준 검증을 실행하고 다음 최소 작업을 선택한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 이슈 번호·목표·현재 상태·첫 명령을 설명할 수 있다
- [ ] 사용자의 기존 변경이 보존된다
