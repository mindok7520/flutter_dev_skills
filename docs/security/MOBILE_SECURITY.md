...
- 필요한 권한만 요청하고 거절 경로를 제공한다
- 디버그 가능·테스트 광고 ID·개발 endpoint를 출시에서 제거한다
- 루팅·탈옥 탐지는 위험 신호이며 최종 권한 검증은 서버에 둔다

## 실행 절차

1. Android manifest와 iOS 권한 설명·entitlement를 검토한다
2. 백업·딥링크·웹뷰·스크린샷·클립보드 사용을 확인한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 잠금·백그라운드·권한 철회 후 민감 정보가 노출되지 않는다
- [ ] 서명된 release 패키지로 검증했다
