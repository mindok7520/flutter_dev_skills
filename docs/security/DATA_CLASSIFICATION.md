...
- 공개·내부·개인·고민감·인증 비밀로 분류한다
- 미분류 데이터는 편의상 공개로 취급하지 않는다
- 영수증·기기 식별자·IP도 맥락에 따라 민감하다

## 실행 절차

1. 필드별 목적·소유자·보관 위치·전송 대상·삭제 기간을 목록화한다
2. 등급별 접근과 암호화·마스킹 정책을 적용한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 새 이벤트와 SDK 수집 필드가 목록에 있다
- [ ] 필요 없는 데이터가 수집되지 않는다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

분류만 하고 실제 경로를 검사하지 않으면 누락된다. 캐시·백업·크래시 보고까지 추적한다.
