...
공유 Dart 코드와 네이티브·웹 기능의 차이를 격리한다.
...
- dart:io와 웹 전용 API를 공통 UI에 직접 가져오지 않는다
- 조건부 import 또는 어댑터를 사용한다
- 지원·미지원·권한 필요를 타입과 화면에서 구분한다

## 실행 절차

1. 기능별 플랫폼 지원 표와 계약을 작성한다
2. 실제 구현과 테스트 대역을 구성 지점에서 연결한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 각 대상의 컴파일·권한 거절·미지원 경로를 검증한다
- [ ] 잘못된 플랫폼 호출이 런타임까지 숨지 않는다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

최저 공통 기능만 쓰면 경험이 제한된다. 플랫폼 특화 기능은 공통 계약을 유지하면서 별도 UX로 제공한다.
