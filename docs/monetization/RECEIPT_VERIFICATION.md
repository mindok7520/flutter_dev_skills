...
영수증·구매 토큰을 신뢰 가능한 공급자 기록과 대조한다.
...
- 검증은 서버에서 수행한다
- 앱 ID·상품·환경·거래 ID·상태·시간·계정 연결을 확인한다
- 토큰과 원본 영수증을 일반 로그에 남기지 않는다

## 실행 절차

1. 공급자 공식 검증 API와 자격을 분리한다
2. 거래 ID의 유일 제약과 원자적 원장 갱신을 구현한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 위조·다른 앱·sandbox/운영 혼합·이미 사용한 거래를 거부한다
- [ ] 공급자 장애 시 대기 후 재검증한다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

클라이언트 서명 검사만으로 전체 거래 상태를 보장하지 못한다. 환불·철회와 서버 알림까지 반영한다.
