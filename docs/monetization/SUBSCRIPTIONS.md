...
- 자동 갱신 해제와 즉시 권리 만료를 구분한다
- 유예·보류·환불·철회·업그레이드·다운그레이드를 모델링한다
- 기기 시계를 권리의 근거로 사용하지 않는다

## 실행 절차

1. 공급자 상태와 내부 권리 상태의 변환 표를 만든다
2. 웹훅과 주기 재조정으로 누락 이벤트를 복구한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 웹훅 중복·역전·누락과 날짜 경계를 검증한다
- [ ] 변경된 플랜의 청구·적용 시점을 사용자에게 정확히 표시한다
