...
광고 사업자 요구와 사용자 선택을 초기화 순서에 반영한다.
...
- UMP 등 선택한 SDK의 공식 동의 흐름을 따른다
- 연령·지역·OS 추적 허용 상태를 별도로 관리한다
- 철회 후 캐시 광고와 대기 요청의 처리를 정의한다

## 실행 절차

1. 동의 정보 갱신→필요한 UI→광고 요청 가능 여부 확인 순서를 검토한다
2. 설정에서 개인정보 선택을 다시 열 수 있게 한다
...
동의 UI를 표시했다는 것만으로 모든 추적이 허용되지 않는다. 공급자·관할별 범위를 확인한다.
