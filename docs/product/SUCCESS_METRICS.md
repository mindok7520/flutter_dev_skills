...
- 핵심 작업 완료율·재방문·오류 없는 세션을 함께 본다
- 광고 클릭률만 최적화하지 않고 오클릭·이탈·신고를 제한 지표로 둔다
- 개인 식별이 없는 집계와 동의 범위를 우선한다

## 실행 절차

1. 지표별 분자·분모·시간 창·중복 제거·제외 규칙을 정의한다
2. 베이스라인과 목표·실험 기간·최소 표본을 기록한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 봇·테스트 계정·중복 이벤트가 분리된다
- [ ] 사용자 삭제 요청이 원천 데이터와 연결된다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

적은 표본의 차이를 성과로 단정하지 않는다. 지표 수집 비용과 개인정보 위험이 가치보다 큰 경우 수집하지 않는다.
