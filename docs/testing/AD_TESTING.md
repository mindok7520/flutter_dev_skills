...
광고 수명·동의·배치가 앱 사용을 해치지 않는지 확인한다.
...
- 공식 테스트 광고 ID와 테스트 기기를 사용한다
- 자체 실제 광고 클릭을 하지 않는다
- 동의 전 SDK 요청과 추적을 금지하는 정책을 구현한다

## 실행 절차

1. 동의 거절·철회·광고 없음·타임아웃·오프라인을 재현한다
2. 화면 이탈·회전·재진입에서 광고 객체 수명을 확인한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 광고 실패가 핵심 작업을 막지 않는다
- [ ] 보상 이벤트 중복이 부당한 권리 지급을 만들지 않는다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

광고 제공은 플랫폼·지역·연령에 따라 다르다. 미지원 대상에서 안전하게 숨기고 성공처럼 기록하지 않는다.
