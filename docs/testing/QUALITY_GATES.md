# 병합 품질 기준

## 목적과 적용 범위

변경을 병합하기 전에 필요한 최소 증거를 정의한다.

이 문서는 대상 Flutter 프로젝트에 적용할 기본 정책이다. `lib/`·앱 테스트·Dart 도구 명령은 대상 프로젝트의 경로이며 이 자료 저장소에 앱 구현이 있다는 뜻이 아니다. 제품별 담당자·수치·공급자는 관련 이슈와 실행 계획에 확정한다. 아래 점검 항목은 수행 절차이며 이미 통과했다는 기록이 아니다.

## 설계와 규칙

- 대상 앱의 기존 기준과 변경 위험에 맞춰 구조·포맷·정적 분석·관련 단위/위젯 테스트를 검증한다. 웹을 지원하면 웹 release 빌드를, 다른 플랫폼은 해당 대상의 빌드와 필요한 기기 검증을 수행한다.
- 플랫폼·결제·보안·성능 변경은 해당 분야 검증을 추가한다
- UI 변경은 합의한 요구·화면 명세와 [실행 화면 검토](../design/VISUAL_REVIEW.md)를 연결한다. 대표 화면 크기·문구 길이·상태·입력 경로를 실제로 확인하고, 모션·셰이더는 축소 모션과 대체 표현도 검증한다. 골든 이미지 일치만으로 사용성과 디자인 품질을 통과시키지 않는다.
- 실패·건너뜀·환경 부족을 통과로 바꾸지 않는다

## 실행 절차

1. 대상 앱의 검증 명령을 실행한다. 선택형 도구를 도입했고 실제 파일이 있다면 `dart run tool/verify.dart`를 사용한다. 자료 저장소의 문서 변경은 루트 `AGENTS.md`의 Python 검증을 따른다.
2. PR에 실제 명령·결과·미실행 항목과 위험을 적는다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] CI quality 작업과 PR 정책 검사가 통과한다
- [ ] 이슈·수락 기준·실행 계획·문서가 변경과 연결된다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

기기 검증이 필요한 변경을 웹 빌드만으로 승인하지 않는다. 알려진 기존 실패도 소유자와 해결 시점 없이 영구 면제하지 않는다.

실패가 확인되면 통과 조건을 낮추지 말고 최소 재현과 영향 범위를 남긴다. 동작 변경은 관련 테스트와 문서를 함께 수정한다. 여러 세션이 필요하면 [실행 계획](../exec-plans/TEMPLATE.md)에 다음 행동을 구체적으로 적는다.

## 연결 문서와 최신성

[문서 지도](../README.md)에서 관련 분야를 선택한다. 기술·정책 근거와 확인 날짜는 [출처 목록](../SOURCES.md)에 있다. 별도 표시가 없는 설계 규칙은 이 저장소의 권장 기본값이며 공식 규격의 강제 요구나 제품 출시 보증이 아니다.
