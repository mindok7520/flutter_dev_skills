...
- 서비스 의존성·우선순위·연락·접근·복구 순서를 문서화한다
- 다중 지역·고가용성은 요구와 비용에 맞춰 선택한다
- 앱의 오프라인 UI를 서버 재해 복구와 혼동하지 않는다

## 실행 절차

1. 장애 시나리오별 전환·데이터 복구·DNS·자격·결제 재조정을 계획한다
2. 격리된 리허설로 실제 복구 시간과 누락을 확인한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 담당자 부재에도 실행 가능한 명령과 접근 경로가 있다
- [ ] 복구 후 데이터·권한·관측이 정상이다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

자동 전환은 잘못된 쓰기 중복과 데이터 손상을 확대할 수 있다. 단일 작성자·조정 정책을 먼저 설계한다.
