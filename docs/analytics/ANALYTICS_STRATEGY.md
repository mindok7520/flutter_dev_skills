...
- 초기 베이스는 분석 공급자가 없다
- 목표 지표에 기여하지 않는 이벤트를 수집하지 않는다
- 동의·삭제·보관과 스키마를 먼저 설계한다

## 실행 절차

1. 핵심 작업 완료와 이탈을 정의한다
2. 허용 필드·집계·샘플링·품질 검사를 구현한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 테스트 이벤트와 운영 이벤트가 분리된다
- [ ] 추적 거절 시 비필수 전송이 없다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

세밀한 행동 추적은 비용과 재식별 위험을 높인다. 먼저 집계 지표로 해결 가능한지 확인한다.
