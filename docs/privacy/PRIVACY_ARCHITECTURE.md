...
- 기본 베이스는 분석·광고 추적을 수집하지 않는다
- 필요한 데이터만 수집하고 목적 외 사용을 제한한다
- SDK의 자동 수집도 제품의 데이터 처리로 관리한다

## 실행 절차

1. 필드·목적·보관·공유·사용자 제어를 데이터 목록에 기록한다
2. 동의·철회·접근·삭제 경로를 설계한다
3. 변경 이유, 영향 파일, 실행한 명령·환경·결과를 해당 이슈의 실행 계획에 기록한다. 적용하지 않은 항목은 이유와 후속 작업을 남긴다.

## 검증과 완료 증거

- [ ] 동의 이전과 철회 이후의 전송을 확인한다
- [ ] 삭제가 캐시·원장·백업 정책과 연결된다
- [ ] 문서의 결정과 실제 코드·설정이 일치하며, 미측정·미구현 항목을 명확히 구분했다.

## 실패 대응과 절충

개인정보 보호는 체크박스 하나로 끝나지 않는다. 국가·연령·업종별 적용 법과 스토어 정책을 출시 전에 다시 확인한다.
