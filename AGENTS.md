...
- 새 UI/UX 설계와 실질적인 화면·탐색·모션 변경은 [디자인 작업 절차](docs/design/DESIGN_WORKFLOW.md)를 따른다. 기존 자료를 읽은 뒤 필요한 사용자 목표·취향·제약을 먼저 질문하고 응답을 기다린다. 방향을 제안하고 사용자 선택을 확인한 뒤 앱 구현을 시작한다. 이미 명시된 답변·승인·위임은 재사용하며 같은 승인을 반복 요청하지 않는다.
- AI 실행 프롬프트와 개정한 분야 지침의 영어 본문은 [공통 실행 계약](docs/agent/PROMPT_CONTRACT.md)을 따른다. 사용자 질문·설명·최종 답변과 README는 한국어로 유지한다. 영어의 보편적인 성능 우위를 가정하지 않는다.
